...
bool Ist8310::init() {
    ...
    if (!writeReg(ist8310::kRegAvg, 0x24)) return false;
    if (!writeReg(ist8310::kRegPdcntl, 0xC0)) return false;  // PDCNTL normal（ArduPilot）
    return true;
}
...
bool Ist8310::read(MagSample& out) {
    ...
    MagSample m = ist8310Decode(raw);   // ⚠️ 装配方向标定留待实物
    if (!m.valid) return false;          // 契约：无效返回 false，out 不变
    out = m;
    return true;
}

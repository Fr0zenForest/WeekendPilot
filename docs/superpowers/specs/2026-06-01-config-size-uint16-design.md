...
当前 `sizeof(ControllerConfig)=226`，余量仅 29B。多个在途/已落地功能（起落架 LandingGearConfig、Mahony ki 预留、ADRC LadrcConfig）都在挤这 29B。255 是**自定义序列化格式的字段宽度限制，与 NVS flash 容量无关**（NVS 分区有 20KB+，配置才占 ~0.2KB）。本 spec 把 size 扩成 uint16（上限→65535），一次性拆掉这个迟早要拆的雷。

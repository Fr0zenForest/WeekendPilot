...
## ⚠️ 启用 WP_HAS_GPS 前必须先做（最终 Opus 评审遗留，Important）

> 这几项不阻塞合并（GPS 当前门控关闭、无消费方），但 **在翻开 `WP_HAS_GPS` 让任何导航功能吃 GPS 数据之前必须解决**，否则有飞行安全隐患：

1. **GnssNmea::read() 会把陈旧定位当新鲜成功读数返回**（`gnss_nmea.cpp:18-26`）。`NmeaParser` 的 `valid` 是粘滞的：GGA fix>0 置位后只有 RMC 'V' 会清除。GPS 中途掉线（无字节 / 只剩 GGA 但 fix 丢失）时 `read()` 会无限返回冻结的最后定位。**修法：** read() 里跟踪本次是否真有新句（`fresh` 标志），返回 `fresh && s.valid`；并给 `GnssSample` 加时间戳让上层判超时。
2. **kGpsBaud=38400 已对数据手册核实**（手册明写"默认38400bps"），无需改；但换波特率配置时记得 M10 不同变体默认值不同。

这两项属阶段 3 导航接线时的前置条件，已记入项目记忆。
